...
def temporal_non_maximum_suppression(predictions, nms_threshold, max_after_nms=100):
    ...
    if len(predictions[0]) > 3: # may add vid_name, cap, ...
        tothers = []
        rothers = []
        for i in range(3, len(predictions[0])):
            tmp = [e[i] for e in predictions]
            tothers.append(tmp)
            rothers.append([])
    ...
    while len(tstart) > 1 and len(rscore) < max_after_nms:  # max 100 after nms
        ...
        while idx < len(tstart):  # compare with every prediction in the list.
            if compute_temporal_iou([tstart[0], tend[0]], [tstart[idx], tend[idx]]) > nms_threshold:
                ...
                tend.pop(idx)
                tscore.pop(idx)
                if tothers is not None:
                    for i in range(len(tothers)):
                        tothers[i].pop(idx)
                # print("--------------------------------")
        ...
        if tothers is not None:
            for i in range(len(tothers)):
                rothers[i].append(tothers[i].pop(0))
    ...
    if tothers is not None:
        for i in range(len(predictions_after_nms)):
            predictions_after_nms[i] = predictions_after_nms[i] + [rothers[j][i] for j in range(len(rothers))]
    # import pdb; pdb.set_trace() 
